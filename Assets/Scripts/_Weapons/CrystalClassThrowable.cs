...
public class CrystalClassThrowable : BaseWeaponClass {
    ...
    public void ActivateWeapon(Vector3 targetLocation)
    {
        ...
        rigidbody.velocity = (targetLocation - transform.position) * weaponItem.AttackSpeedOrDuration;
        ...
        if (inventory.TryUnequipItem(weaponItem))
            inventory.TryTakeItem(weaponItem);
    ...
    {
        Collider[] collidersInRange = Physics.OverlapSphere(transform.position, weaponItem.StatEffectOrRadius);
        ...
        foreach (Collider collider in collidersInRange)
        {
           CharacterStats gotComponent = collider.GetComponent<CharacterStats>();
            ...
            if (gotComponent != null)
            {
                for (int i = 0; i < 1; i++)
                {
                    gotComponent.TakeDamage(weaponItem.WeaponDamage * weaponDamageModfier);
                }
            }
        }
    }
